...
Что делает:
  1. Берёт из Supabase лоты с cadastral_number, но без координат (lat IS NULL)
  2. Для каждого запрашивает НСПД API
  3. Обновляет Supabase: lat, lon, boundary_geojson, cadastral_value,
     cadastral_area, cadastral_category, cadastral_permitted_use,
     cadastral_price_ratio, encumbrance_type
...
import math
import re
CN_FULL = re.compile(r"^\d{2}:\d{2}:\d{5,7}:\d+$")
CN_PARTIAL = re.compile(r"^\d{2}:\d{2}:\d{5,7}:[А-Яа-яA-Za-z\d]+$")  # ЗУ, ЗУ1 и т.п.
CN_QUARTER = re.compile(r"^(\d{2}:\d{2}:\d{5,7})")


def is_valid_cadastral(cn: str) -> bool:
    """Проверяет формат кадастрового номера XX:XX:XXXXXXX:XXXX."""
    return bool(CN_FULL.match(cn.strip()))


def is_partial_cadastral(cn: str) -> bool:
    """Проверяет частичный формат (XX:XX:XXXXXXX:ЗУ1)."""
    return bool(CN_PARTIAL.match(cn.strip())) and not CN_FULL.match(cn.strip())


def extract_quarter(cn: str) -> str | None:
    """Извлечь кадастровый квартал XX:XX:XXXXXXX из кадастрового номера."""
    m = CN_QUARTER.match(cn.strip())
...
def fetch_nspd_multi(raw_cad: str) -> dict | None:
    ...
    Для частичных (ЗУ) — ищет по кварталу."""
    parts = split_cadastral(raw_cad)
    for cn in parts:
        if is_valid_cadastral(cn):
            result = fetch_nspd(cn)
            if result:
                return result
        elif is_partial_cadastral(cn):
            # Частичный кадастр (ЗУ1) — ищем по кварталу
            quarter = extract_quarter(cn)
            if quarter:
                log.info("  Частичный кадастр %s → квартал %s", cn, quarter)
                result = fetch_nspd_quarter(quarter)
                if result:
                    return result
        time.sleep(1)
    return None
...
def fetch_nspd_quarter(quarter: str) -> dict | None:
    """Запрос НСПД по кадастровому кварталу — возвращает центроид квартала."""
    try:
        resp = sess.get(NSPD_URL, params={"query": quarter, "limit": 1}, timeout=15)
        if resp.status_code == 403:
            log.warning("  403 — пауза 30 сек...")
            time.sleep(30)
            resp = sess.get(NSPD_URL, params={"query": quarter, "limit": 1}, timeout=15)
            if resp.status_code == 403:
                return None
        ...
        result["_geometry"] = feature.get("geometry")
        result["_categoryName"] = props.get("categoryName", "")
        result["_is_quarter"] = True  # Пометка: координаты приблизительные
        return result
    except Exception as e:
        log.error("  НСПД квартал %s: %s", quarter, e)
        return None
...
def _extract_ring(geom: dict) -> list | None:
    """Извлечь внешнее кольцо координат из GeoJSON geometry."""
    gtype = geom.get("type", "")
...
def _extract_polygon_wgs84(geom: dict) -> dict | None:
    """Конвертировать geometry НСПД → GeoJSON Polygon WGS-84."""
...
def parse_nspd(raw: dict, price: float) -> dict:
    ...
    if geom:
        ...
        if boundary:
            update["boundary_geojson"] = boundary
...
def main():
    # Берём лоты с кадастровым номером но без координат
    ...
    lots = resp.data
    log.info("Найдено %d лотов без координат (с кадастровым номером)", len(lots))
